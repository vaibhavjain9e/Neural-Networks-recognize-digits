...
class linear_layer:
    ...
    def __init__(self, input_D, output_D):
        ...
        # np.random.normal() with mean as 0 and standard deviation as 0.1
...
class relu:
    ...
    def forward(self, X):
        ...
        X_zeros = np.zeros((X.shape[0], X.shape[1]))
        forward_output = np.maximum(X_zeros, X)
        self.mask = np.maximum(X_zeros, X)
        return forward_output
    ...
    def backward(self, X, grad):
        ...
        for i in range(0, len(self.mask)):
            for j in range(0, len(self.mask[i])):
                if self.mask[i][j] > 0:
                    self.mask[i][j] = 1
...
class tanh:
    ...
    def backward(self, X, grad):
        ...
        h = np.tanh(X)
        backward_output = np.multiply(grad, 1 - np.square(h))
        return backward_output
...
class dropout:
    ...
    def forward(self, X, is_train):
        ...
        #  Forward pass is provided only fro understanding.
        ...
        if hasattr(module, 'params'):
            for key, _ in module.params.items():
                ...
                if _alpha > 0.0:
                    ...
                    # Update momentum using the formula:
...
def main(main_params, optimization_type="minibatch_sgd"):
    ...
    for t in range(num_epoch):
        ...
        for i in range(int(np.floor(N_train / minibatch_size))):
            ...
            # forward
            # Call the forward methods of every layer in the model in order
